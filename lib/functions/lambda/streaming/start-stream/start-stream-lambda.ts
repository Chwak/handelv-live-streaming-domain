...
export const handler = async (event: {
  ...
  initTelemetryLogger(event, { domain: "live-streaming-domain", service: "start-stream" });
  ...
  if ((stream.status as string) !== 'SCHEDULED') throw new Error('Stream cannot be started');
  ...
  const updateResult = await client.send(
    new UpdateCommand({
      TableName: LIVE_STREAMS_TABLE,
      Key: { streamId },
      UpdateExpression: 'SET #st = :status, startedAt = :now, updatedAt = :now',
